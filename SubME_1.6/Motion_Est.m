...
if(strcmp(opts.SearchMethod, 'Log'))
    Search = @(Block, img_ref, xc, yc, SearchLimit) LogSearch(Block, img_ref, xc, yc, SearchLimit, UseTaylorExp);
else
    Search = @(Block, img_ref, xc, yc, SearchLimit) FullSearch(Block, img_ref, xc, yc, SearchLimit, UseTaylorExp);
end
...
for i = 1:length(yc_range)
    for j = 1:length(xc_range)
        ...
        [MVy1, MVx1]     = Search(Block, img_ref, xc, yc, SearchLimit);
...
%MVx = MVx/InterpolationLevel;
%MVy = MVy/InterpolationLevel;